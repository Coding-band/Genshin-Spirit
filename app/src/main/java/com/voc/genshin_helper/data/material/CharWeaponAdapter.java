...
public class CharWeaponAdapter extends RecyclerView.Adapter<CharWeaponAdapter.CharWeaponViewHolder> {

    private List<Materials> materialsList;
    Context context;
    Activity activity;

    public CharWeaponAdapter(Context context, List<Materials> materialsList, Activity activity){
    ...
    @NonNull
    @Override
    public CharWeaponViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new CharWeaponViewHolder(LayoutInflater.from(context).inflate(R.layout.item_cal_img,parent,false));
    }

    @Override
    public void onBindViewHolder(CharWeaponViewHolder holder, int position) {
        ...
        int column = width_curr/150;
        int size_per_img = width_curr/column;

        if(width_curr/column > 150){
            size_per_img = 150;
        }
    ...
    public class CharWeaponViewHolder extends RecyclerView.ViewHolder {


        public TextView item_name,item_weekly_tv;
        public ImageView item_icon,item_press_mask;

        public CharWeaponViewHolder(@NonNull View itemView) {
            super(itemView);
            AlphaAnimation buttonClick = new AlphaAnimation(1F, 0.8F);
            item_icon = itemView.findViewById(R.id.item_cal_img);
            item_name = itemView.findViewById(R.id.item_cal_tv);
